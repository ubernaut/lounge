...
$(function() {
	...
	Settings.prototype = {
		...
		mergeFromServer: function(data) {
			for (let key in this.serverOptions) {
				this.sync(key, false);
			}
		...
		loadFromLocalStorage: function() {
			var localOptions = JSON.parse(window.localStorage.getItem("settings"));

			for (var k in localOptions) {
				this.set(k, localOptions[k]);
			}
		},
	...
	// Settings handlers
	(function() {
		var userStylesHead = $(document.head).find("#user-specified-css");

		/*
		for (var i in options) {
			if (i === "userStyles") {
				if (!/[?&]nocss/.test(window.location.search)) {
					userStylesHead.html(options[i]);
				}
				settings.element.find("#user-specified-css-input").val(options[i]);
			} else if (i === "highlights") {
				settings.element.find("input[name=" + i + "]").val(options[i]);
			} else if (i === "theme") {
				$("#theme").attr("href", "themes/" + options[i] + ".css");
				settings.element.find("select[name=" + i + "]").val(options[i]);
			} else if (options[i]) {
				settings.element.find("input[name=" + i + "]").prop("checked", true);
			}
		}

		settings.element.on("change", "input, select, textarea", function() {
			var self = $(this);
		...
		if (("Notification" in window)) {
			...
			settings.set("desktopNotifications", false);
			desktopNotificationsCheckbox.attr("disabled", true);
			desktopNotificationsCheckbox.attr("checked", false);
		}
	...
	chat.on("msg", ".messages", function(e, target, msg) {
		...
		if (msg.highlight || (settings.get("notifyAllMessages") && msg.type === "message")) {
